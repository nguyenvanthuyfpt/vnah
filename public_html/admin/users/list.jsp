...
<%if(!bean.getUsername().equals(me.getUsername())){%>
    <td width="20px">
      <img style="border:0px" src="<%=contextPath%>/images/delete.png" title="<bean:message key="action.delete" bundle="<%=interfaces%>"/>" onClick="javascript:if(messageDelete())postAjax('users','tdMainBody',anchor + ':_DELETE:id:<%=userid%>:groupID:<%=bean.getGroupID()%>');">
    </td>                                           
<%}%>
<%}%>
</logic:notEqual>
