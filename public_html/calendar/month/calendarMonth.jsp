...
function showTT(){
    post('agenda',anchor + ':_CALENDAR_MONTH');
}
...
function getDataByDate(obj,date){
    ...
    postAjax('agenda','calendarExt',anchor + ':_DETAIL_CALENDAR:timeEvent:'+date);
}
...
<tr>
                ...
                <a href="javascript:post('agenda',anchor + ':_HOME_CURRENT');"><bean:message key="agenda.label.today" bundle="<%=interfaces%>"/>,<bean:message key="agenda.form.edit.timeEvent" bundle="<%=interfaces%>"/> <%=beanDate.getDate(beanDate.getCurrentSqlDate())%>  <bean:message key="agenda.label.month" bundle="<%=interfaces%>"/> <%=beanDate.getMonth(beanDate.getCurrentSqlDate())%> <bean:message key="agenda.label.year" bundle="<%=interfaces%>"/> <%=beanDate.getYear(beanDate.getCurrentSqlDate())%></a>
                </div>
                </td>
</tr>
</table>
...
<logic:iterate name="BEvents" id="bean" type="com.form.calendar.agenda.FAgenda">
    ...
    <a href="javascript:post('agenda',anchor + ':_SELECT_WEEK:id_week:<%=bean.getDayId()%>')"><bean:message key="agenda.label.week" bundle="<%=interfaces%>"/> <%=i/7+1%></a>
    </td>
    <%}%>
    <%if(bean.getMonth(bean.getDay())!=monthSelect){%>
                <td class="empty" valign="top" >
                   ...
                   <logic:iterate name="bean" property="eventsInDay"  id="beanInDay" indexId="k" type="com.form.calendar.agenda.FAgenda">
                    <%if(k<3){%>
                    <div  class="calendarPortlet" >
                          <bean:write name="beanInDay" property="times" />-<bean:write name="beanInDay" property="toTimes" /> : <bean:write name="beanInDay" property="what" />
                    </div>  
                  <%}%>
                   <%if(k==3){%>
    ...
    <%}else if((i+1)%7==0){%>
               <td class="weekend" valign="top" >
                   ...
                   <logic:iterate name="bean" property="eventsInDay"  id="beanInDay" indexId="k" type="com.form.calendar.agenda.FAgenda">
                    <%if(k<3){%>
                    <div  class="calendarPortlet" >
                          <span onclick="javascript:prepareCalendar('<%=beanInDay.getTimes()%>','<%=beanInDay.getTimeEvent()%>',<%=beanInDay.getEventId()%>)"><bean:write name="beanInDay" property="times" />-<bean:write name="beanInDay" property="toTimes" /> : <bean:write name="beanInDay" property="what" /></span>
                    </div>  
                   <%}%><%if(k==3){%>
                    <div><a href="javascript:getDataByDate(this,'<%=beanInDay.getTimeEvent()%>')">+ Detail</a></div>
                   <%}%>
