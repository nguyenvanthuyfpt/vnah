...
function dTree(objName,nodeClass) {
...
// check id is child of PID
dTree.prototype.isChild = function(id,PID) {
  var parent_id = id;
  var i=this.aNodes.length-1;
  while(i>0){
      if(parent_id==this.aNodes[i].id){
        if(PID==this.aNodes[i].pid){        
          return true;
        }else{
          parent_id=this.aNodes[i].pid;  
          i=this.aNodes.length-1;
        }
      }else{
        i--;
      }
...
dTree.prototype.node = function(node, nodeId) {
        var dTreeNode = 'dTreeNode';
        if(this.nodeClass!=null) dTreeNode = this.nodeClass;
	var str = '<div class="' + dTreeNode + '">' + this.indent(node, nodeId);

	if (this.config.useIcons) {

		if (!node.icon) node.icon = (this.root.id == node.pid) ? this.icon.root : ((node._hc) ? this.icon.folder : this.icon.node);
//                if (!node.icon) node.icon = (this.root.id == node.pid) ? this.icon.node : ((node._hc) ? this.icon.folder : this.icon.node);
...
//                        node.icon = this.icon.node;
	...
	if (node._hc) {
		...
		str += '<div id="d' + this.obj + nodeId + '" class="clip" style="white-space:nowrap;display:' + ((this.root.id == node.pid || node._io) ? 'block' : 'none') + ';">';
