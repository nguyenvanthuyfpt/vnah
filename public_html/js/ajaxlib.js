...
function createREQ() {
    var PROGIDS = new Array("Msxml2.XMLHTTP.7.0", "Msxml2.XMLHTTP.6.0", "Msxml2.XMLHTTP.5.0", "Msxml2.XMLHTTP.4.0", "MSXML2.XMLHTTP.3.0", "MSXML2.XMLHTTP", "Microsoft.XMLHTTP");
    var req = null;
...
function getParams(postForm) {
    ...
    if (postForm && postForm.elements)
        for (i = 0;i < postForm.elements.length;i++) {
            ...
            if (obj.type == 'text' || obj.type == 'textarea' || obj.type == 'select-one' || obj.type == 'select-multiple' || obj.type == 'radio' || obj.type == 'checkbox' || obj.type == 'password' || obj.type == 'hidden') {
                var value = '';
...
function doAjax(url, query, objResult, reqtype, getxml, functionCallBack) {
    ...
    var myreq = createREQ();
    if (myreq == null)
        alert("Web browser does not support AJAX.");
...
function showWindow(winName) {
    ...
    var win = getObj(winName);
    if (win == null)
        win = getObj(winName, self.parent.document);
    try {
        var _maxWidth = 0;
        for (i = 0;i < win.childNodes.length;i++) {
            try {
                if (_maxWidth < win.childNodes[i].width)
                    _maxWidth = win.childNodes[i].width;
            }
            catch (ex) {
            }
        }
        ...
        var scroll_left = window.pageXOffset ? window.pageXOffset : body.scrollLeft;
        ...
        win.style.left = scroll_left + (docwidth - _maxWidth - 100) / 3 + 'px';
    }
    catch (ex) {
    }
    finally {
