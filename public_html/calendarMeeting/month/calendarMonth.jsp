...
function showTT(){
    post('agendaMeeting',anchor + ':_CALENDAR_MONTH');
}
function prepareCalendar(timeValue,dateValue,id){
        ...
        openWindow('agendaMeeting',anchor + ':_PREPARED_CREATE'+values);
}
...
function getDataByDate(obj,date){
    ...
    postAjax('agendaMeeting','calendarExt',anchor + ':_DETAIL_CALENDAR:timeEvent:'+date);
}
...
<tr>
                ...
                <a href="javascript:post('agendaMeeting',anchor + ':_HOME_CURRENT');"><bean:message key="agenda.label.today" bundle="<%=interfaces%>"/>,<bean:message key="agenda.form.edit.timeEvent" bundle="<%=interfaces%>"/> <%=beanDate.getDate(beanDate.getCurrentSqlDate())%>  <bean:message key="agenda.label.month" bundle="<%=interfaces%>"/> <%=beanDate.getMonth(beanDate.getCurrentSqlDate())%> <bean:message key="agenda.label.year" bundle="<%=interfaces%>"/> <%=beanDate.getYear(beanDate.getCurrentSqlDate())%></a>
                </div>
                </td>
</tr>
</table>
...
<%int i=0;%>
<logic:iterate name="BEvents" id="bean" type="com.form.calendarMeeting.agenda.FAgendaMeeting">
    <%if(i%7==0){%>
    <tr><td style="text-align:center" >
    <a href="javascript:post('agendaMeeting',anchor + ':_SELECT_WEEK:id_week:<%=bean.getDayId()%>')"><bean:message key="agenda.label.week" bundle="<%=interfaces%>"/> <%=i/7+1%></a>
    </td>
    <%}%>
    <%if(bean.getMonth(bean.getDay())!=monthSelect){%>
                <td class="empty" valign="top" >
                   ...
                   <logic:iterate name="bean" property="eventsInDay"  id="beanInDay" indexId="k" type="com.form.calendarMeeting.agenda.FAgendaMeeting">
                    <%if(k<3){%>
