...
<td style="border:#CCCCCC 1px solid;">
        ...
        <div>
         ...
         <logic:notEqual name="bean" property="fileName" value="">
            <div>
                                 <a href="javascript:post('docReviewSend',anchor + ':_DRAFT:reviewId:<%=bean.getReviewId()%>');remove('docReviewSend',anchor);remove('docReviewSend','reviewId');">
                                 <bean:write name="bean" property="fileName" />
                                 </a>
                                 <logic:equal name="docAssign" property="type" value="2">
                                    <span id="tranfer<%=i%>"><input type="button" style="width:80px"   value="<bean:message key="cmd.doc.review.confer" bundle="<%=interfaces%>"/>" oncdivck="javascript:this.src='images/loading.gif';postAjax('docMainAssignSend','tranfer<%=i%>',anchor + ':_TRANFER_DOCS:reviewId:<%=bean.getReviewId()%>')" />
                                    </span>
                                 </logic:equal>
              </div>
             </logic:notEqual>
