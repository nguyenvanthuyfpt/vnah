...
<logic:notEmpty name="docAssignRecv" property="groupMembers" >
  <bean:define name="docAssignRecv" property="groupMembers" id="groupMembers" type="java.lang.String"/>
                 ...
                 <%  
                   checkeds = statusIdsName.indexOf("," + beanTranfer.getId() + "#" + bean.getId()+ "," )<0?"":"checked"; %>
                    <td class="<%=(i%2>0?"tdcontent":"tdcontent1")%>" align="center" >
                          <input type="checkbox" <%=checkeds%> name="disposeGroup" id="disposeGroup" onclick="checkChecked(document.docAssignRecv.disposeGroup,<bean:write name="beanTranfer" property="id"/>,<%=bean.getId()%>);memberChecked(document.docAssignRecv.disposeGroup,3)" value="<bean:write name="beanTranfer" property="id"/>#<bean:write name="bean" property="id"/>"/>
                    </td>                
                </logic:iterate>                
            </tr>
        </logic:iterate>
