...
public class DMucdo extends DSqlDisability {
    public boolean isExist(Connection conn, FSeed seed) throws EException {
        final String LOCATION = "->isExist()";
        boolean result = false;
        FMucdo bean = (FMucdo)seed;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            pstmt = conn.prepareStatement(SQL_SELECT_MUCDO_INFORMATION);
    ...
    public FMucdo getRecordByID(Connection cnn, FSeed seed) throws EException {
        final String LOCATION = this.toString() + "getRecordByID()";
        PreparedStatement prstm = null;
        ResultSet rs = null;
        FMucdo bean = new FMucdo();
        bean = (FMucdo)seed;
        try {
            prstm = cnn.prepareStatement(SQL_SELECT_MUCDO_BY_ID);
    ...
    public boolean haveChild(Connection conn, FSeed seed) throws EException {
        ...
        ResultSet rs = null;
        try {
            pstmt = conn.prepareStatement(SQL_SELECT_MUCDO_HAVECHILD);
    ...
    public boolean insert(Connection cnn, FSeed seed) throws EException {
        ...
        try {
            ...
            result = execute(cnn, SQL_INSERT_MUCDO, params) > 0;
    ...
    public boolean update(Connection cnn, FSeed seed) throws EException {
        ...
        try {
            ...
            result = execute(cnn, SQL_UPDATE_MUCDO, params) > 0;
    ...
    public boolean delete(Connection cnn, FSeed seed) throws EException {
        ...
        FMucdo bean = (FMucdo)seed;
        return 0 < delete(cnn, TABLE_MUCDO, MUCDO_ID + EQUAL + bean.getId());
    }
    ...
    public FBeans getMultiRecords(Connection cnn, int idDepartment) throws EException {
        ...
        ResultSet rs = null;
        try {
            prstm = cnn.prepareStatement(SQL_SELECT_MUCDO);
            rs = prstm.executeQuery();
            String members = ",";
            FMucdo bean = null;
    ...
    public FMucdo getInformation(ResultSet rs) throws EException {
        final String LOCATION = "->getInformation()";
        FMucdo requiretype = new FMucdo();
        try {
            requiretype.setId(rs.getInt(MUCDO_ID));
            requiretype.setCode(rs.getString(MUCDO_CODE));
            requiretype.setName(rs.getString(MUCDO_NAME));
            requiretype.setParentID(rs.getInt(MUCDO_PARENT_ID));
        } catch (SQLException sqle) {
            if (AppConfigs.APP_DEBUG)
                throw new EException(LOCATION, sqle);
        } finally {
