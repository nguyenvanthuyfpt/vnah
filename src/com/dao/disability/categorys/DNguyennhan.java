...
public class DNguyennhan extends DSqlDisability {
    public boolean isExist(Connection conn, FSeed seed) throws EException {
        ...
        FNguyennhan bean = (FNguyennhan)seed;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            pstmt = conn.prepareStatement(SQL_SELECT_NGUYENNHAN_INFORMATION);
    ...
    public FNguyennhan getRecordByID(Connection cnn, FSeed seed) throws EException {
        final String LOCATION = this.toString() + "getRecordByID()";
        PreparedStatement prstm = null;
        ResultSet rs = null;
        FNguyennhan bean = new FNguyennhan();
        bean = (FNguyennhan)seed;
        try {
            prstm = cnn.prepareStatement(SQL_SELECT_NGUYENNHAN_BY_ID);
    ...
    public boolean haveChild(Connection conn, FSeed seed) throws EException {
        ...
        ResultSet rs = null;
        try {
            pstmt = conn.prepareStatement(SQL_SELECT_NGUYENNHAN_HAVECHILD);
    ...
    public boolean insert(Connection cnn, FSeed seed) throws EException {
        ...
        try {
            ...
            result = execute(cnn, SQL_INSERT_NGUYENNHAN, params) > 0;
    ...
    public boolean update(Connection cnn, FSeed seed) throws EException {
        ...
        try {
            ...
            result = execute(cnn, SQL_UPDATE_NGUYENNHAN, params) > 0;
    ...
    public boolean delete(Connection cnn, FSeed seed) throws EException {
        ...
        FNguyennhan bean = (FNguyennhan)seed;
        return 0 < delete(cnn, TABLE_NGUYENNHAN, NGUYENNHAN_NNHAN_ID + EQUAL + bean.getId());
    }


    public FNguyennhan getInformation(ResultSet rs) throws EException {
        final String LOCATION = "->getInformation()";
        FNguyennhan requiretype = new FNguyennhan();
        try {
            requiretype.setId(rs.getInt(NGUYENNHAN_NNHAN_ID));
            requiretype.setCode(rs.getString(NGUYENNHAN_NNHAN_CODE));
            requiretype.setName(rs.getString(NGUYENNHAN_NNHAN_NAME));
            requiretype.setParentID(rs.getInt(NGUYENNHAN_NNHAN_PARENT_ID));
        } catch (SQLException sqle) {
            if (AppConfigs.APP_DEBUG)
                throw new EException(LOCATION, sqle);
        } finally {
    ...
    public FBeans getMultiRecords(Connection cnn, int idDepartment) throws EException {
        ...
        ResultSet rs = null;
        try {
            prstm = cnn.prepareStatement(SQL_SELECT_NGUYENNHAN);
            rs = prstm.executeQuery();
            String members = ",";
            FNguyennhan bean = null;
