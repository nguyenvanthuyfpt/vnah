...
public class DDonvi extends DSqlDisability {
    public boolean isExist(Connection conn, FSeed seed) throws EException {
        final String LOCATION = "->isExist()";
        boolean result = false;
        FDonvi bean = (FDonvi)seed;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            pstmt = conn.prepareStatement(SQL_SELECT_DONVI_INFORMATION);
    ...
    public FDonvi getRecordByID(Connection cnn, FSeed seed) throws EException {
        final String LOCATION = this.toString() + "getRecordByID()";
        PreparedStatement prstm = null;
        ResultSet rs = null;
        FDonvi bean = new FDonvi();
        bean = (FDonvi)seed;
        try {
            prstm = cnn.prepareStatement(SQL_SELECT_DONVI_BY_ID);
    ...
    public boolean haveChild(Connection conn, FSeed seed) throws EException {
        ...
        ResultSet rs = null;
        try {
            pstmt = conn.prepareStatement(SQL_SELECT_DONVI_HAVECHILD);
    ...
    public boolean insert(Connection cnn, FSeed seed) throws EException {
        ...
        try {
            ...
            result = execute(cnn, SQL_INSERT_DONVI_DONVI, params) > 0;
    ...
    public boolean update(Connection cnn, FSeed seed) throws EException {
        ...
        try {
            ...
            result = execute(cnn, SQL_UPDATE_DONVI_DONVI, params) > 0;
    ...
    public boolean delete(Connection cnn, FSeed seed) throws EException {
        ...
        FDonvi bean = (FDonvi)seed;
        return 0 < delete(cnn, TABLE_DONVI, DONVI_DONVI_ID + EQUAL + bean.getId());
    }
    ...
    public FBeans getMultiRecords(Connection cnn, int idDepartment) throws EException {
        ...
        ResultSet rs = null;
        try {
            prstm = cnn.prepareStatement(SQL_SELECT_DONVI);
            rs = prstm.executeQuery();
            String members = ",";
            FDonvi bean = null;
    ...
    public FDonvi getInformation(ResultSet rs) throws EException {
        final String LOCATION = "->getInformation()";
        FDonvi requiretype = new FDonvi();
        try {
            requiretype.setId(rs.getInt(DONVI_DONVI_ID));
            requiretype.setCode(rs.getString(DONVI_CODE));
            requiretype.setName(rs.getString(DONVI_NAME));
            requiretype.setParentID(rs.getInt(DONVI_PARENT_ID));
        } catch (SQLException sqle) {
            if (AppConfigs.APP_DEBUG)
                throw new EException(LOCATION, sqle);
        } finally {
