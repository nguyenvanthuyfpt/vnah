...
public class DDieuKien extends DSqlDisability {
    public boolean isExist(Connection conn, FSeed seed) throws EException {
        final String LOCATION = "->isExist()";
        boolean result = false;
        FDieuKien bean = (FDieuKien)seed;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            pstmt = conn.prepareStatement(SQL_SELECT_DIEUKIEN_INFORMATION);
    ...
    public FDieuKien getRecordByID(Connection cnn, 
                                   FSeed seed) throws EException {
        final String LOCATION = this.toString() + "getRecordByID()";
        PreparedStatement prstm = null;
        ResultSet rs = null;
        FDieuKien bean = new FDieuKien();
        bean = (FDieuKien)seed;
        try {
            prstm = cnn.prepareStatement(SQL_SELECT_DIEUKIEN_BY_ID);
    ...
    public boolean haveChild(Connection conn, FSeed seed) throws EException {
        ...
        ResultSet rs = null;
        try {
            pstmt = conn.prepareStatement(SQL_SELECT_DIEUKIEN_HAVECHILD);
    ...
    public boolean insert(Connection cnn, FSeed seed) throws EException {
        ...
        try {
            ...
            result = execute(cnn, SQL_INSERT_DIEUKIEN_DIEUKIEN, params) > 0;
    ...
    public boolean update(Connection cnn, FSeed seed) throws EException {
        ...
        try {
            ...
            result = execute(cnn, SQL_UPDATE_DIEUKIEN_DIEUKIEN, params) > 0;
    ...
    public boolean delete(Connection cnn, FSeed seed) throws EException {
        ...
        FDieuKien bean = (FDieuKien)seed;
        return 0 < 
            delete(cnn, TABLE_DIEUKIEN, DIEUKIEN_DIEUKIEN_ID + EQUAL + bean.getId());
    }
    ...
    public FBeans getMultiRecords(Connection cnn, 
        ...
        ResultSet rs = null;
        try {
            prstm = cnn.prepareStatement(SQL_SELECT_DIEUKIEN);
            rs = prstm.executeQuery();
            String members = ",";
            FDieuKien bean = null;
    ...
    public FDieuKien getInformation(ResultSet rs) throws EException {
        final String LOCATION = "->getInformation()";
        FDieuKien requiretype = new FDieuKien();
        try {
            requiretype.setId(rs.getInt(DIEUKIEN_DIEUKIEN_ID));
            requiretype.setCode(rs.getString(DIEUKIEN_CODE));
            requiretype.setName(rs.getString(DIEUKIEN_NAME));
            requiretype.setParentID(rs.getInt(DIEUKIEN_PARENT_ID));
        } catch (SQLException sqle) {
            if (AppConfigs.APP_DEBUG)
                throw new EException(LOCATION, sqle);
        } finally {
